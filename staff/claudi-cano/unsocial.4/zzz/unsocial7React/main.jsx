...
class PasswordInput extends Component {
    ...
    render() {
        ...
        return <div style={{ display: 'flex' }}>
            <input type={this.state.type} id={this.props.id} style={{ width: '100%', boxSizing: 'border-box', paddingRigth: '18px' }} />
            <span
...
function Register(props) {
    return <section>
        ...
        <form onSubmit={event => {
            ...
            const { target: { name: { value: name }, email: { value: email }, username: { value: password }, ['password-repeat']: { value: passwordRepeat } } } = event
        }}>
...
class App extends Component {
    ...
    render() {
        return <div>
            ...
            {this.state.view === 'login'
                && <Login onLoggedIn={() => this.setState({ view: 'home' })}
                    onRegisterLink={() => this.setState({ view: 'register' })} />}
            {this.state.view === 'register'
                && <Register onLoginLink={() => this.setState({ view: 'login' })} />}
